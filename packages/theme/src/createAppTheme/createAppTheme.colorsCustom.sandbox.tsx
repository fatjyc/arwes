...
import { type AppTheme, type AppThemeSettingsPalette, createAppTheme } from '@arwes/theme'
    ...
    <Fragment>
      ...
      {(Object.keys(t.colors) as unknown as Array<keyof AppTheme['colors']>).map((colorName) => (
        <div key={colorName}>
          ...
          {(
            Object.keys(t.colors[colorName]) as unknown as Array<
              keyof AppTheme['colors'][typeof colorName]
            >
          ).map((variant) => (
            <div key={variant}>
              <h3>{variant}</h3>
                  ...
                  .map((_, index) => (
                    <div
                      ...
                      title={`theme.colors.${String(colorName)}.${String(
                        variant
                      )}(${index}) = ${t.colors[colorName][variant](index)}`}
                    >
                      <code>{index}</code>
                    </div>
                  ))}
