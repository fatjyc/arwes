...
describe('dynamic list', () => {
  ...
  test('Should return item in lch dynamic list within valid value ranges', () => {
    ...
    expect(themeColor(1)).toBe('lch(100% 100% 360 / 1)')
  })
...
describe('function', () => {
  ...
  test('Should return item in lch creator with valid ranges', () => {
    const themeColor = createThemeColor({
      color: 'lch',
      create: (i) => [50, 50 * i, 200 * i, 0.5 * i]
    })
    expect(themeColor(0)).toBe('lch(50% 0% 0 / 0)')
    expect(themeColor(1)).toBe('lch(50% 50% 200 / 0.5)')
    expect(themeColor(2)).toBe('lch(50% 100% 360 / 1)')
    expect(themeColor(3)).toBe('lch(50% 100% 360 / 1)')
  })
