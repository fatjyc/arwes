...
const defaultProps: Required<
  Pick<PuffsProps, 'padding' | 'xOffset' | 'yOffset' | 'radiusInitial' | 'radiusOffset' | 'sets'>
> = {
  padding: 50,
  xOffset: [0, 0],
  yOffset: [-10, -100],
    ...
    const animatorSubscription = (node: AnimatorNode): void => {
      ...
      switch (state) {
        case entering: {
          ...
          const { color, quantity, padding, xOffset, yOffset, radiusInitial, radiusOffset, sets } =
          ...
          canvasControl = animate(canvas, { opacity: [0, 1] }, { duration: duration?.enter })
        ...
        case exiting: {
          canvasControl?.cancel()
          canvasControl = animate(canvas, { opacity: [1, 0] }, { duration: duration?.exit })
          break
        }
