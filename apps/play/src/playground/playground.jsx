...
window.noxtron.setupPlayground(({ emotion, Icon }) => {
  ...
  const isLocationNext = ['next.arwes.dev', '127.0.0.1', 'localhost'].find((host) =>
    window.location.host.includes(host)
  )

  const Link = (props) => <a css={{ display: 'flex', alignItems: 'center' }} {...props} />

  const LinkIcon = (props) => <Icon css={{ marginRight: '5px' }} path={props.path} size="1em" />
  ...
  const versionLink = (
    <Link
      ...
      target="github"
    >
      <LinkIcon path={ICONS.mdiLabelOutline} />{' '}
      {isLocationNext ? 'Version Next' : `v${LERNA_SETTINGS.version}`}
    </Link>
  )
